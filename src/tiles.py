...
class Tile(object):
    ...
    def __init__(self, type, x, y, timer=0, replace_tile=""):
        ''' Simple initalizer.
...
class RandomTile(Tile):
    ...
    def __init__(self, type, x, y, timer=0, replace_tile=""):
        ...
        ''' 
        super(RandomTile, self).__init__(type, x, y, timer, replace_tile)
...
def make_tile(type, x, y):
    ...
    if not constants.DEACTIVATE_RANDOM_TEXTURES:
        if constants.IMAGES[type].random:
            return RandomTile(type, x, y, timer, replace_tile)
        else:
            return Tile(type, x, y, timer, replace_tile)
    else:
        return Tile(type, x, y, timer, replace_tile)